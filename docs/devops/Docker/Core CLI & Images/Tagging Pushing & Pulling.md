...
Given your .NET Core and Python projects:

- **CI/CD**: Build once in Jenkins, tag with `:v1.2.3`, `:latest`, and commit SHA, then push to ACR or Docker Hub. [youtube](https://www.youtube.com/watch?v=Cjr9bIrePSM)
- **Kubernetes**: Reference specific versions in deployment YAML (`image: myapp:v1.2.3`), never `:latest` in production. [augmentedmind](https://www.augmentedmind.de/2022/05/15/docker-image-tag-best-practices/)
- **Multi-stage builds**: Build in a `sdk` image, tag the final `aspnet` runtime image, and push only the slim version. [docs.docker](https://docs.docker.com/reference/cli/docker/image/push/)
